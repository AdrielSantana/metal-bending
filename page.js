...
export async function page(prog) {
  ...
  try {
    ...
    if (prog.init) await prog.init(rt);
    ...
    window.__result = { name: prog.name, res: prog.W + "x" + prog.H, g: rt.G, dispatches: rt.ND + 2, ...burst, err: c.err, heap_ok: c.heap_ok, bump: c.bump, joins: c.joins, sum: prog.sum ? prog.sum(c.pixels) : undefined, root_tag: (c.root[1] >>> 24) & 127, done: c.root[2], mismatches: mism, near: close, of: N, bad, checksum: sum, slab: rt.SLAB, lanes: rt.LANES, depth: rt.DEPTH, errors: rt.errors };
